...
const DialogForm = ({ updateData, id }) => {
  ...
  useEffect(() => {
    axios
      .get('http://eventcompany.somee.com/Departamentos/ListarDepartamentos')
  ...
  const DireccionesAction = e => {
    ...
    axios.post('https://localhost:44393/api/Direcciones/InsertarDirecciones', payload2).then(r => {
      if (r.data.data.codeStatus == 2) {
        ...
        if (r.data.data.codeStatus == 3) {
          ...
          if (r.data.data.codeStatus == 1) {
            toast.success('Registro ingresado con exito!', {
              duration: 4000
            })
            updateData()
            handleClose()
